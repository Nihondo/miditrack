...
4. Optionally adjust the speed multiplier and/or semitone transpose with the compact −/＋ steppers beside "WAVをダウンロード" (Download WAV); the middle values can also be typed directly. This changes the MIDI itself (tempo and note numbers), so it carries through to every render, WAV download, and MIDI download until changed back
5. Inspect the piano roll at any time after the MIDI is ready. Use its separate −/＋ controls at the lower-right corner to zoom the time axis and scroll horizontally when zoomed in. Click "適用して試聴" (Apply & Audition) to render; you can then seek by clicking/dragging the piano roll, using its keyboard controls, or dragging the standard `<audio>` scrubber
