...
- The "適用して試聴" (Apply & Audition) button beside the SoundFont dropdown renders a `.wav` via [midi2wav.sh](../midi2wav.sh) (fluidsynth + SoundFont) and plays it through an `<audio>` element. Two segmented controls above the piano roll group back/forward and return-to-start/play-pause; they stay synchronized with Arrow Left/Right, Home or Command+Left, and Space respectively
- Download the edited `.mid`, or the rendered `.wav` itself, with the buttons immediately below the standard audio player — a real deliverable, not just a preview. The uploaded/converted original is never modified. A text field next to the buttons lets you set the base filename for both downloads (pre-filled with the currently loaded file's name); the bulk variations ZIP below also uses whatever name is set here, both for the ZIP itself and for every file inside it
...
7. Click "MIDIをダウンロード" (Download MIDI) or "WAVをダウンロード" (Download WAV) immediately below the standard audio player to save the result
8. Optionally expand "バリエーションをまとめて生成" (Generate variations in bulk) below the player, enter comma-separated speed factors and pitch (semitone) values, choose whether "ZIPにMIDIも含める" (Include MIDI in ZIP) is checked, and click "バリエーションをZIPでダウンロード" (Download variations as ZIP) to get every combination's WAV (and MIDI) bundled in one ZIP, named from the same filename field (no audition render needed first — it doesn't touch the whole-song speed/pitch setting in the toolbar)
