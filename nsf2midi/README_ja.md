...
FDS、N163、S5B（FME-7/Sunsoft 5B）、VRC6矩形波チャンネルについては、その
チャンネルの最初のノートオン時点の状態を記録した`timbre`ブロックも
sidecarへ含まれます: FDSは64ステップ波形とマスタ音量、N163は波形RAMと
アクティブチャンネル数、S5Bはトーン/ノイズ混合状態とハードウェア
エンベロープ設定、VRC6はデューティ比レジスタです。各ブロックには単純な
ヒューリスティックでその状態から選んだGM Program番号1つ
（`gmProgramCandidate`）も含まれます（FDSは波形の粗さ、N163はアクティブ
チャンネル数——実機では多いほどデチューン気味のボーカル的な質感になる
ことが知られています、S5Bはハードウェアエンベロープまたはノイズ混合の
有無、VRC6は`DutyProgramChangeEnabled`が実際に送出するのと同じデューティ
→Program対応）。これはメタデータに過ぎず、音高・ゲートの変換や
Program Changeの実送出自体を変えるものではありません——vgm2midiの
OPN/OPM/OPL `fm`候補がVGMで担うのと同じ、判断材料としての位置づけです
（`docs/chip-support.md`参照）。
