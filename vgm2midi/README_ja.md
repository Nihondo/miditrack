...
- FM音源のパラメータは簡略化されています — MIDIにはFM音色のネイティブなモデルが無いため、YM2203/YM2608/YM2612/YM2151のチャンネルは単純なノートとして近似されます。YM2612とYM2608はOPN2/OPNA共通のphase generator基準周波数（YM2203のOPN基準に対する1/2を含む）を共有し、YM2608内蔵SSGもYM2203のSSGとは異なるOPNA向けの分周比を使います。どちらの周波数基準もlibvgmでレンダリングした実データ音声と照合済みです。その基準の上で、アルゴリズム、キーオンマスク、トータルレベル、明示的に書き込まれたオペレータ倍率を、0.5倍を含む明確な2の累乗の共通オクターブ補正に使用します。共通する2の累乗倍率を持たない比率、デチューン、エンベロープ、聴感上のミッシングファンダメンタルは、従来どおり生のF-Numberによる近似のままです
