...
exports.prepareVGMPlayback = prepareVGMPlayback;
const VGM_SAMPLE_RATE = 44100;
function countSamples(commands) {
...
function getCommandBody(commands) {
    const endIndex = commands.findIndex(command => command.type === 'end');
    return endIndex === -1 ? commands.slice() : commands.slice(0, endIndex);
}
function appendCommands(output, commands) {
    for (const command of commands) {
        output.push(command);
    }
}
function appendUntilSample(output, commands, currentSamples, targetSamples) {
...
function requireLoopCommands(data, bodyLength) {
 ...
 */
function prepareVGMPlayback(data, options = {}) {
    ...
    if (options.durationSeconds !== undefined) {
        ...
        while (currentSamples < targetSamples) {
            ...
            currentSamples = appendUntilSample(outputCommands, loopCommands, currentSamples, targetSamples);
        }
    }
    else {
