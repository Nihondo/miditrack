...
exports.writeC140SoundFont = writeC140SoundFont;
...
function writeC140SoundFont(vgmData, sampleNotes, outputPath) {
    if (vgmData.header.c140Clock <= 0 || sampleNotes.size === 0)
        return 0;
...
function buildC140Rom(blocks) {
    ...
    for (const block of romBlocks) {
        if (block.payload.length >= 8)
            romSize = Math.max(romSize, block.payload.readUInt32LE(0));
    }
    if (romSize === 0 || romSize > 16 * 1024 * 1024)
...
function collectC140Samples(vgmData, sampleNotes, rom) {
    if (rom.length === 0 || vgmData.header.c140Type !== C140_SYSTEM2_TYPE)
        return [];
    const registers = new Uint8Array(0x200);
    const definitions = new Map();
    const active = new Array(24).fill(false);
    for (const command of vgmData.commands) {
        if (command.type !== 'chip_write' || command.chip !== 'C140' || (command.instance ?? 0) !== 0)
            continue;
        ...
        if (!sampleNotes.has(`c140_sample_${id}`) || definitions.has(id))
            continue;
        definitions.set(id, {
            bank,
            start,
...
function mapSystem2Address(bank, address) {
...
function decodeSystem2Pcm(rom, start, end) {
...
function calculateC140SourceRate(clock, frequency) {
...
function buildSoundFont(samples) {
    const smplParts = [];
    const headers = [];
    let cursor = 0;
    for (const sample of samples) {
        const pcm = Buffer.alloc(sample.pcm.length * 2);
        for (let index = 0; index < sample.pcm.length; index += 1)
            pcm.writeInt16LE(sample.pcm[index], index * 2);
        smplParts.push(pcm);
...
function buildSampleHeader(name, start, end, loopStart, loopEnd, sampleRate, rootKey) {
...
function preset(name, bagIndex) { return Buffer.concat([fixedText(name, 20), u16(0), u16(SF2_DRUM_BANK), u16(bagIndex), Buffer.alloc(12)]); }
function instrument(name, bagIndex) { return Buffer.concat([fixedText(name, 20), u16(bagIndex)]); }
function bag(generatorIndex, modulatorIndex) { return Buffer.concat([u16(generatorIndex), u16(modulatorIndex)]); }
function generator(oper, amount) { return Buffer.concat([u16(oper), u16(amount)]); }
function chunk(id, data) { return Buffer.concat([Buffer.from(id), u32(data.length), data, ...(data.length % 2 === 0 ? [] : [Buffer.alloc(1)])]); }
function list(type, contents) { return chunk('LIST', Buffer.concat([Buffer.from(type), ...contents])); }
function text(value) {
...
function fixedText(value, length) { const result = Buffer.alloc(length); result.write(value.slice(0, length - 1), 'ascii'); return result; }
function u16(...values) { const result = Buffer.alloc(values.length * 2); values.forEach((value, index) => result.writeUInt16LE(value & 0xFFFF, index * 2)); return result; }
function u32(...values) { const result = Buffer.alloc(values.length * 4); values.forEach((value, index) => result.writeUInt32LE(value >>> 0, index * 4)); return result; }
