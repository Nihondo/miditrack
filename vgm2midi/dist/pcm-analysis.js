...
exports.romDataBlockForMetadata = romDataBlockForMetadata;
exports.romBytesForRange = romBytesForRange;
exports.c140ROMBytesForRange = c140ROMBytesForRange;
exports.analyzePCMValues = analyzePCMValues;
exports.pcmTimbreForAnalysis = pcmTimbreForAnalysis;
exports.analyzeSigned8BitPCM = analyzeSigned8BitPCM;
exports.signed8BitPCMAnalysisForROMRange = signed8BitPCMAnalysisForROMRange;
exports.c219MuLawAnalysisForROMRange = c219MuLawAnalysisForROMRange;
exports.c14012BitPCMAnalysisForROMRange = c14012BitPCMAnalysisForROMRange;
exports.c140CompressedPCMAnalysisForROMRange = c140CompressedPCMAnalysisForROMRange;
exports.ym2608ADPCMBAnalysis = ym2608ADPCMBAnalysis;
exports.segaPCMAnalysisForTrack = segaPCMAnalysisForTrack;
exports.c140PCMAnalysisForVoice = c140PCMAnalysisForVoice;
exports.segaPCMBankBaseAddress = segaPCMBankBaseAddress;
exports.segaPCMDurationSamples = segaPCMDurationSamples;
exports.c140DurationSamples = c140DurationSamples;
exports.c140ROMAddress = c140ROMAddress;
...
const C140_C219_EXTERNAL_BANK_REGISTERS = [0x1F7, 0x1F1, 0x1F3, 0x1F5];
/** sidecarのdata block参照から実ROM blockを探す。 */
function romDataBlockForMetadata(vgmData, dataBlock) {
    return (vgmData.dataBlocks ?? []).find(candidate => candidate.type === dataBlock.bankType
        && (candidate.instance ?? 0) === dataBlock.bankInstance
        && candidate.blockId === dataBlock.blockId);
}
/** ROM block内のbyteアドレス範囲を安全に取得する。 */
function romBytesForRange(vgmData, dataBlock, endAddress) {
    ...
    if (dataBlock.blockOffset + sourceByteLength > block.payload.length - 8)
        return undefined;
...
function c140ROMBytesForRange(vgmData, dataBlock, endAddress) {
    ...
    if (sourceStart + sourceByteLength > block.payload.length)
        return undefined;
    return block.payload.subarray(sourceStart, sourceStart + sourceByteLength);
}
/** 数値sample列を均等に間引き、正規化された波形特徴量へ変換する。 */
function analyzePCMValues(format, sourceByteLength, sampleCount, maximumAmplitude, sampleAt) {
    if (sampleCount === 0)
        return undefined;
    ...
    const normalize = (value) => Math.round((value / maximumAmplitude) * 1000000) / 1000000;
    return {
        format, sourceByteLength, analyzedSampleCount,
        ...(sampleCount > analyzedSampleCount ? { isDownsampled: true } : {}),
...
function pcmTimbreForAnalysis(analysis) {
...
function analyzeSigned8BitPCM(samples, sign = 1) {
    return analyzePCMValues('signed-8bit-pcm', samples.length, samples.length, 128, index => sign * (samples[index] - 0x80));
}
/** 単一ROM blockに完全に収まる符号付き8-bit PCMの基本波形特徴量を返す。 */
function signed8BitPCMAnalysisForROMRange(vgmData, dataBlock, endAddress, sign = 1) {
...
function c219MuLawAnalysisForROMRange(vgmData, dataBlock, endAddress, sign) {
    const samples = romBytesForRange(vgmData, dataBlock, endAddress);
...
function c14012BitPCMAnalysisForROMRange(vgmData, dataBlock, endAddress) {
    const samples = c140ROMBytesForRange(vgmData, dataBlock, endAddress);
    if (samples === undefined)
        return undefined;
    const sampleCount = Math.floor(samples.length / 2);
    return analyzePCMValues('signed-12bit-be-pcm', samples.length, sampleCount, 2048, index => samples.readInt16BE(index * 2) >> 4);
}
/** C140圧縮PCMのMAME互換テーブル復号を解析に用いる。 */
function c140CompressedPCMAnalysisForROMRange(vgmData, dataBlock, endAddress) {
    const samples = c140ROMBytesForRange(vgmData, dataBlock, endAddress);
    if (samples === undefined)
        return undefined;
    const sampleCount = Math.floor(samples.length / 2);
...
function ym2608ADPCMBAnalysis(vgmData, dataBlock, sourceByteLength) {
    if (dataBlock === undefined || sourceByteLength === undefined)
        return undefined;
...
function segaPCMAnalysisForTrack(vgmData, dataBlock, events) {
    if (dataBlock?.bankType !== 0x80 || dataBlock.romStartAddress === undefined)
        return undefined;
...
function c140PCMAnalysisForVoice(vgmData, dataBlock, startAddress, endAddress, mode) {
    if (startAddress !== dataBlock?.bankOffset)
        return undefined;
...
function segaPCMBankBaseAddress(vgmData, control) {
...
function segaPCMDurationSamples(vgmData, address, endPage, frequency, sampleRate) {
    if (frequency === 0)
        return undefined;
    const clock = vgmData.header.segaPCMClock & vgm_chip_metadata_1.CLOCK_MASK;
    if (clock === 0)
        return undefined;
...
function c140DurationSamples(vgmData, start, end, frequency, sampleRate) {
    if (end <= start || frequency === 0)
        return undefined;
    const inputClock = vgmData.header.c140Clock & vgm_chip_metadata_1.CLOCK_MASK;
    if (inputClock === 0)
        return undefined;
...
function c140ROMAddress(vgmData, c140Registers, channel, bank, address) {
