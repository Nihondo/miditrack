...
exports.handleYM2413Write = handleYM2413Write;
exports.handleYM2413RhythmModeWrite = handleYM2413RhythmModeWrite;
exports.updateYM2413Frequency = updateYM2413Frequency;
exports.handleYM2413KeyAndFrequencyWrite = handleYM2413KeyAndFrequencyWrite;
exports.commitYM2413KeyOn = commitYM2413KeyOn;
exports.handleYM2413VolumeWrite = handleYM2413VolumeWrite;
exports.ym2413Velocity = ym2413Velocity;
exports.ym2413RhythmVelocity = ym2413RhythmVelocity;
...
const YM2413_RHYTHM_NOTES = [36, 42, 38, 45, 49];
// BD, HH, SD, TOM, CYM -> $0E bit masks
const YM2413_RHYTHM_KEY_BITS = [0x10, 0x01, 0x08, 0x04, 0x02];
function handleYM2413Write(host, cmd, currentTime, activeNotes, cmdIndex) {
    ...
    if (reg >= 0x00 && reg <= 0x07) {
        ...
        if (reg === 0x01)
            host.hasYM2413CustomCarrierMultiple = true;
    ...
    if (reg >= 0x20 && reg <= 0x28) {
        handleYM2413KeyAndFrequencyWrite(host, reg - 0x20, currentTime, activeNotes, cmdIndex, data, cmd.instance ?? 0);
        return;
    }
...
function handleYM2413RhythmModeWrite(host, data, currentTime, activeNotes) {
    ...
    for (let i = 0; i < YM2413_RHYTHM_KEY_BITS.length; i++) {
        ...
        const key = `ym2413_rhythm_${i}`;
        if ((newBits & bit) !== 0) {
            (0, event_output_1.noteOnPercussion)(host, key, ym2413RhythmVelocity(host, i), currentTime, activeNotes, YM2413_RHYTHM_NOTES[i]);
        }
...
function updateYM2413Frequency(host, channel, currentTime, activeNotes, cmdIndex, data, instance) {
    const key = `ym2413_${channel}`;
    const state = host.channels.get(key);
    ...
    const isSplitUpdate = host.isOPNMultiByteFreqUpdate(cmdIndex, 'YM2413', 0, otherReg, instance);
...
function handleYM2413KeyAndFrequencyWrite(host, channel, currentTime, activeNotes, cmdIndex, data, instance) {
    const key = `ym2413_${channel}`;
    const state = host.channels.get(key);
...
function commitYM2413KeyOn(host, channel, currentTime, activeNotes) {
    const key = `ym2413_${channel}`;
    const state = host.channels.get(key);
    if (state.active)
        return;
...
// per-channel) volume.
function handleYM2413VolumeWrite(host, channel, data, currentTime, activeNotes) {
...
function ym2413Velocity(volume) {
    return Math.max(1, Math.min(100, Math.round(100 - volume * 6.6)));
}
function ym2413RhythmVelocity(host, index) {
