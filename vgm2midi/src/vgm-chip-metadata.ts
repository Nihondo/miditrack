...
export const CLOCK_MASK = 0x3FFFFFFF;
export const DUAL_CHIP_FLAG = 0x40000000;

/** Y8950のFM部と共有されない、今回変換対象外のDelta-T ADPCMレジスタ。 */
export const Y8950_ADPCM_REGISTERS: ReadonlySet<number> = new Set([
  0x07,
...
export const STREAM_DEVICE_CHIPS = [
 ...
 */
export const COMMAND_CHIPS: Record<number, ChipCommandMetadata> = {
...
export function unsupportedOperandCount(command: number, version: number): number | undefined {
  if (command >= 0x30 && command <= 0x3F) return 1;
  if (command >= 0x40 && command <= 0x4E) return version <= 0x0160 ? 1 : 2;
  if (command === 0x4F) return 1;
  if (command >= 0x55 && command <= 0x5F) return 2;
  if (command === 0x64) return 3;
  if (command === 0x68) return 11;
  if (command === 0x90 || command === 0x91) return 4;
  if (command === 0x92) return 5;
  if (command === 0x93) return 10;
  if (command === 0x94) return 1;
  if (command === 0x95) return 4;
  if (command >= 0xA1 && command <= 0xBF) return 2;
  if (command >= 0xC0 && command <= 0xDF) return 3;
  if (command >= 0xE0 && command <= 0xFF) return 4;
  return undefined;
}
