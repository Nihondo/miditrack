...
import { VGMHeader, VGMCommand, VGMData, VGMDataBlock, VGMDiagnostics } from './types';
import {
  CLOCK_MASK,
  COMMAND_CHIPS,
  STREAM_DEVICE_CHIPS,
  Y8950_ADPCM_REGISTERS,
  unsupportedOperandCount,
} from './vgm-chip-metadata';

interface VGMCompressionTable {
  compressionType: number;
  subType: number;
  bitsDecompressed: number;
  bitsCompressed: number;
  values: number[];
}

export class VGMParser {
  ...
  private skipBytes(byteCount: number): void {
    this.ensureAvailable(byteCount);
    this.position += byteCount;
  }

  /** VGM圧縮table blockを読み、後続compressed data block用に登録する。 */
  private readCompressionTable(payload: Buffer): VGMCompressionTable {
    if (payload.length < 6) throw new Error('Invalid VGM compression table: header is truncated');
  ...
  private readPackedValue(payload: Buffer, bitOffset: number, bitCount: number): number {
  ...
  private decompressDataBlock(payload: Buffer, tables: Map<string, VGMCompressionTable>): Buffer {
    if (payload.length < 10) throw new Error('Invalid compressed VGM data block: header is truncated');
    const compressionType = payload[0];
    ...
    for (let offset = 0; offset < outputSize; offset += valueBytes) {
      ...
      if (compressionType === 0) {
        if (subType === 0) value = packedValue + baseValue;
        else if (subType === 1) {
          if (bitsCompressed > bitsDecompressed) throw new Error('Invalid compressed VGM data block: shift subtype width');
          value = (packedValue << (bitsDecompressed - bitsCompressed)) + baseValue;
        } else if (subType === 2) value = table!.values[packedValue] ?? (() => { throw new Error('Invalid compressed VGM data block: table index'); })();
        else throw new Error(`Invalid compressed VGM data block: unsupported bit-pack subtype ${subType}`);
      ...
      if (valueBytes === 2) output[offset + 1] = (value >>> 8) & 0xFF;
    }
    return output;
  }

  private readHeaderUInt32LE(offset: number, dataOffset: number): number {
    if (offset + 4 > dataOffset || offset + 4 > this.buffer.length) return 0;
    return this.buffer.readUInt32LE(offset);
  }

  private readHeaderUInt8(offset: number, dataOffset: number): number {
    if (offset + 1 > dataOffset || offset + 1 > this.buffer.length) return 0;
    return this.buffer.readUInt8(offset);
  }

  /** VGM 1.00/1.01の共有FM clockが属する最初のFM writeを境界通り探索する。 */
  private findLegacyFMChip(offset: number, version: number): 'YM2413' | 'YM2612' | 'YM2151' | undefined {
    let position = offset;
    while (position < this.buffer.length) {
      const command = this.buffer[position++];
      if (command === 0x66) return undefined;
      if (command === 0x51) return 'YM2413';
      if (command === 0x52 || command === 0x53) return 'YM2612';
      if (command === 0x54) return 'YM2151';
      if (command === 0x50 || command === 0x4F || command === 0x30 || command === 0x31 || command === 0x3F) { position += 1; continue; }
      if (command >= 0x55 && command <= 0x5F) { position += 2; continue; }
      if (command === 0x61) { position += 2; continue; }
      if (command === 0x62 || command === 0x63 || (command & 0xF0) === 0x70 || (command & 0xF0) === 0x80) continue;
      const operands = this.getUnsupportedCommandOperandCount(command, version);
      if (operands === undefined || position + operands > this.buffer.length) return undefined;
      position += operands;
    }
    return undefined;
  }
  ...
  parseHeader(): VGMHeader {
    ...
    const readChipClock = (offset: number, minimumVersion: number): number => (
      version >= minimumVersion ? this.readHeaderUInt32LE(offset, vgmDataOffset) : 0
    );
  ...
  private parseExtraHeader(): Array<{ chip: string; instance: number; clock: number; volume: number }> {
    if (this.buffer.length < 0xC0) return [];
    const relativeOffset = this.buffer.readUInt32LE(0xBC);
    if (relativeOffset === 0) return [];
    const start = 0xBC + relativeOffset;
    if (start + 12 > this.buffer.length) return [];
    ...
    const entries = new Map<string, { chip: string; instance: number; clock: number; volume: number }>();
    const readList = (offset: number, isClock: boolean): void => {
      if (offset === 0) return;
      // offsets are relative to the offset field itself, not the header start.
      const base = start + (isClock ? 4 : 8) + offset;
      if (base >= this.buffer.length) return;
      const count = this.buffer[base];
      for (let index = 0; index < count; index += 1) {
        const position = base + 1 + index * (isClock ? 5 : 4);
        if (position + (isClock ? 5 : 4) > this.buffer.length) break;
        const rawId = this.buffer[position]; const chip = names[rawId & 0x7F] ?? `Chip${rawId & 0x7F}`;
        ...
        if (isClock) current.clock = this.buffer.readUInt32LE(position + 1) & CLOCK_MASK;
        else current.volume = this.buffer.readUInt16LE(position + 2);
        entries.set(key, current);
      }
    };
    readList(clockOffset, true); readList(volumeOffset, false);
    return [...entries.values()];
  }

  private parseCommandStream(header: VGMHeader): {
    commands: VGMCommand[];
    ...
    let loopCommandIndex: number | undefined;
    const ym2612PcmChunks: Buffer[] = [];
    const dataBlocks: VGMDataBlock[] = [];
    ...
    const dataBlockCounts = new Map<string, number>();
    const compressionTables = new Map<string, VGMCompressionTable>();
    ...
    while (this.position < this.buffer.length) {
      ...
      // y=1 mask; with no YM2203 clock present it unambiguously targets YM2608.
        ...
        const chip = !isOPNSSG
          ? 'AY8910'
          : (header.ym2203Clock & CLOCK_MASK) !== 0 || (header.ym2608Clock & CLOCK_MASK) === 0
            ? 'YM2203'
            : 'YM2608';
      ...
      // ordinary FM and shared control writes are converted by the common OPL handler.
        ...
        cmd === 0x5A || cmd === 0x5B || cmd === 0x5C
        || cmd === 0xAA || cmd === 0xAB || cmd === 0xAC
      ) {
        const chip = COMMAND_CHIPS[cmd].chip;
        const register = this.readUInt8();
        const data = this.readUInt8();
        const instance = cmd >= 0xAA ? 1 : 0;
        if (chip === 'Y8950' && Y8950_ADPCM_REGISTERS.has(register)) {
          commands.push({
      ...
      // Data block
        ...
        if (blockType === 0x00 && instance === 0) ym2612PcmChunks.push(payload);
        commands.push({ type: 'data_block', data: originalType, ...(instance === 1 ? { instance } : {}) });
      }
      ...
      // stable editing-trigger identity while native stems retain real audio.
        ...
        const streamId = this.readUInt8(); const data = this.readUInt8();
        const port = this.readUInt8(); const register = this.readUInt8();
        const targetChip = STREAM_DEVICE_CHIPS[data & 0x7F];
        commands.push({ type: 'stream_setup', streamId, data, port, register, targetChip, targetInstance: (data & 0x80) !== 0 ? 1 : 0, command: cmd });
      } else if (cmd === 0x91) {
        const streamId = this.readUInt8(); const bankId = this.readUInt8();
        const stepSize = this.readUInt8(); const stepBase = this.readUInt8();
        commands.push({ type: 'stream_data', streamId, bankId, stepSize, stepBase, command: cmd });
      } else if (cmd === 0x92) {
        const streamId = this.readUInt8(); const frequency = this.readUInt32LE();
        commands.push({ type: 'stream_frequency', streamId, frequency, command: cmd });
      } else if (cmd === 0x93) {
        const streamId = this.readUInt8(); const address = this.readUInt32LE();
        const mode = this.readUInt8(); const length = this.readUInt32LE();
        ...
        const streamId = this.readUInt8(); const block = this.readUInt16LE(); const flags = this.readUInt8();
        commands.push({ type: 'stream_start_fast', streamId, address: block, blockId: block, data: flags, command: cmd });
      }

      else if (COMMAND_CHIPS[cmd] && (COMMAND_CHIPS[cmd].minVersion === undefined || header.version >= COMMAND_CHIPS[cmd].minVersion!)) {
        const metadata = COMMAND_CHIPS[cmd];
  ...
  private getUnsupportedCommandOperandCount(command: number, version: number): number | undefined {
    return unsupportedOperandCount(command, version);
  }
  ...
  private createDiagnostics(header: VGMHeader, commands: VGMCommand[], unsupportedCommandCount: number): VGMDiagnostics {
    const entries = new Map<string, { chip: string; instance: number; commandCount: number; writeCount: number; streamCount: number; midiSupport: 'full' | 'trigger' | 'none' }>();
    const clockForChip = (chip: string): number => (header.chipClocks?.[chip] ?? ({
    ...
    const add = (chip: string, instance: number, options: { isWrite?: boolean; isStream?: boolean; support?: 'full' | 'trigger' | 'none' } = {}) => {
      const key = `${chip}:${instance}`;
      ...
      if (options.isWrite) entry.writeCount += 1;
      if (options.isStream) entry.streamCount += 1;
      if (support === 'none') entry.midiSupport = 'none';
      entries.set(key, entry);
    };
    const streamTargets = new Map<number, { chip: string; instance: number; support: 'trigger' | 'none' }>();
    for (const command of commands) {
      if (command.type.startsWith('stream_')) {
        ...
        if (target) {
          ...
          if (target.support === 'none' && isStart) unsupportedCommandCount += 1;
        }
        continue;
      }
