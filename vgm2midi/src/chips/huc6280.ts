...
export function handleHuC6280Write(
  host: MidiConverter,
  cmd: VGMCommand,
  currentTime: number,
  ...
  if (reg === 0x01) {
    ...
    for (let index = 0; index < 6; index += 1) updateHuC6280Pan(host, instance, index, currentTime);
    return;
  }
  ...
  if (reg === 0x02) {
    ...
    if (
      ...
      noteOff(host, noiseKey, channel, currentTime, activeNotes);
...
export function updateHuC6280Pan(host: MidiConverter, instance: number, channel: number, currentTime: number): void {
  const key = `huc6280_${instance}_${channel}`; const local = host.channels.get(key)?.balance ?? 0xFF;
  ...
  addPan(host, key, hasLeft, hasRight, currentTime);
}

export function syncHuC6280ToneState(
  host: MidiConverter,
  key: string,
  channel: number,
  currentTime: number,
...
export function syncHuC6280NoiseState(
  host: MidiConverter,
  key: string,
  channel: number,
  currentTime: number,
...
export function updateHuC6280NoiseEnvelope(
  host: MidiConverter,
  key: string,
  channel: number,
  oldVolume: number,
...
export function noteOnHuC6280Noise(
  host: MidiConverter,
  key: string,
  state: ChannelState,
  currentTime: number,
  ...
  noteOnPercussion(host, key, velocity, currentTime, activeNotes, huc6280NoiseNoteForPeriod(state.noisePeriod ?? 0));
}
...
// NFRQ below.)
export function huc6280NoiseNoteForPeriod(rawValue: number): number {
  const normalizedRate = (rawValue & 0x1F) / 31;
  return noiseDrumNote(normalizedRate, false);
}

export function addHuC6280Expression(host: MidiConverter, key: string, volume: number, currentTime: number): void {
  addExpression(host, key, Math.round((volume / 31) * 127), currentTime);
}
...
export function isHuC6280MultiByteFreqUpdate(
  host: MidiConverter,
  cmdIndex: number,
  otherReg: number,
  instance: number
